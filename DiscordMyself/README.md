...
Most people simply resort to emailing themselves, since email supports the majority of attachment types. There is even a shortcut called [Email Myself](https://www.icloud.com/shortcuts/dd6a0881316f478b951b2ef72c8a0d58) to do that! However, this ends up cluttering your inbox and leaving you to manually delete your *self-emails* every time.
...
## How It Works
Just like *Email Myself*, this shortcut lets you send anything to yourself, *through Discord*, from Share Sheet, or the clipboard, just with one tap! You can even select multiple items to be sent at a time (*each item will be sent as an individual message on Discord*)!

**Using with Share Sheet (<img src="https://i.imgur.com/KDyO3ny.png" alt="share-sheet-icon" width="2.5%"/>)**:
- Whether you are scrolling on the web, going through your photos, looking at PDF files, or want to save a map location, if you see the Share Sheet icon, then you can send it to Discord in one tap!
...
This shortcut is essentially a streamlined process of sending content to a specific channel of your choice, from a server that you own, as opposed to either manually opening the app and sending content to the server/channel you want to or using Share Sheet to manually do the same.
...
2. Create your private server (*if you already have a private server, you can skip this step*).
...
4. Create a webhook for the channel you just created and copy its URL.
	...
	- On the newly created webhook that just popped up, click on the *Channel* dropdown, and select the name of the channel you just created.
		- Feel free to change the name of the webhook and give it a profile image.
...
1. **Webhook URL**
	...
	- For this shortcut, a Webhook URL is needed to know where to send the content that you choose to upload.
2. **Character Limit**
...
4. **Preserve Images**
	- This setting allows you to choose whether to preserve an image or not. Here, preserving an image means your image ***will neither be converted nor compressed*** to image formats that Discord supports (*i.e. JPG, PNG, GIF*) and *metadata* is unchanged, as well.
...
5. **Preserve Videos**
	- This setting allows you to choose whether to preserve a video or not. Here, preserving a video means your video ***will not be re-encoded*** to the codec(s) that Discord supports (*ex: H.264*), and *metadata* is unchanged, as well. 
...
## Conclusion
And, that's it! You have successfully set up the **Discord Myself** shortcut on your phone. Now, you can send anything you want from your phone to Discord, either through the Share Sheet or with your clipboard.
...
## Disclaimer
***As a general warning, Discord is not a safe place to store sensitive or identifying information. Please do not send passwords, addresses, or any other sensitive information to your private Discord server. But it is safe to use this shortcut if you want to transfer content that is not sensitive or identifying to you.***
